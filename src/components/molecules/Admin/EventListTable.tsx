...
import { getEventWorkspaceList } from "@/services/admin";
...
function EventListTable() {
  ...
  const {
    ...
    queryKey: ["eventList", currentPage],
    queryFn: () =>
      getEventWorkspaceList({
        page: currentPage,
        limit: pageSize,
      }),
    select: (response) => {
        ...
        <div className="overflow-x-auto  no-scrollbar">
          <div className="inline-block min-w-full align-middle ">
            {isLoading ? (
              ...
              <>
                {eventList && eventList?.data?.length >= 1 ? (
                  <Table
                    headers={headers}
                    pagination={
                      <Pagination
                        ...
                        totalRecords={eventList.total}
                        pageSize={pageSize}
                        onPageChange={handlePageChange}
                      />
                    }
                    tableDesc="Event List Table"
                  >
                    {eventList?.data?.map((d, i) => (
                      <tr key={i} className="hover:bg-gray-50 cursor-pointer">
                        <td className="px-12  py-4 text-sm font-normal text-gray-700 whitespace-nowrap">
                          <img
                            src={getApiMedia(d?.event_profile?.logo || "")}
