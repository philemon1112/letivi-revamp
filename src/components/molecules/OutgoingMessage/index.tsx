...
function OutgoingMessage({ content, user, time, media }: OutgoingMessageProps) {
  ...
  return (
    <div className="group flex gap-2  w-fit max-w-[95%] md:max-w-[75%] lg:max-w-[60%] ml-auto">
      <div className="flex flex-col gap-1  ">
        {media && media?.length > 0 ? (
          <div className="space-y-2 bg-[#377DFF] text-sm text-white rounded-tr-none rounded-lg p-3">
            {media.map((media) => (
              <div
                key={media.id}
          ...
          <TruncatedMessage
            customStyles="mt-2 bg-[#377DFF] text-sm text-white rounded-tr-none rounded-lg p-3"
            maxLength={400}
            description={content}
            buttonColor={"text-black"}
          />
        ...
        <div className="text-[12px] text-end flex justify-end items-center gap-2">
          ...
          <span className="text-[1rem] text-end">
            <BsCheckAll />
          </span>
        </div>
      </div>
