...
function MobileGalleryCard() {
  ...
  return (
    <div className="block lg:hidden col-span-4 my-2 mx-1">
      {/* <div className="bg-white shadow rounded-2xl py-3 px-4">
        ...
        {userPosts && (
          <>
            {userPosts?.length > 0 ? (
              <div>
                <div className="grid grid-cols-3 md:grid-cols-3 gap-1.5 my-2 items-center">
                  {userPosts?.map((item) => (
                    <div key={item?.id}>
                      {item.type === "image" ? (
                        <img
                          className="object-cover object-center w-full mb-0.5 h-16 rounded-lg"
                          src={
