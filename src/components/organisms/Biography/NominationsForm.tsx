...
import { Nomination, ProfessionalFormData } from "@/types/biography";
...
function NominationsForm() {
  const currentUser = useCurrentUser();
  const [nominations, setNominations] = useState<Nomination[]>([]);
  const [loading, setLoading] = useState(false);

  const { data: nominationData, refetch } = useQuery({
    queryKey: ["nominationList"],
    queryFn: async () => {
      if (!currentUser?.id) throw new Error("User ID is undefined");
      return await fetchUserNominations(currentUser.id);
    },
  });
  ...
  const handleAddNomination = () => {
    setNominations([
      ...nominations,
      { nomination: "", country: "", id: 0, created_at: "", updated_at: "" },
    ]);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (res) {
      toast.success("Nominations Updated Successfully");
      refetch();
    }
    setLoading(false);
  };
  ...
  return (
    <div className="px-2 md:px-4 bg-white py-4 md:py-10 my-4 rounded-xl">
      ...
      {nominations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center text-gray-500">
          ...
          <p className="text-sm sm:text-base">
            You haven’t added any Nominations yet.
          </p>
        ...
        nominations.map((nomination, index) => (
          ...
          >
            <div className="w-full pb-2 md:pb-4 grid grid-cols-2 gap-4 lg:p-4">
              <div className="w-full">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange(index, "nomination", e.target.value)
                  }
            ...
            <Trash
              ...
              onClick={() => handleRemoveNomination(index)}
            />
          </div>
        ))
      )}
      ...
      <div className="flex flex-col gap-y-2 sm:flex-row justify-between lg:px-4">
        <Button variant="secondary" size="lg" onClick={handleAddNomination}>
          <IoAddCircle size={24} className="mr-2" /> Add Nomination
        </Button>
        <Button
          variant="primary"
          size="lg"
