...
function SharePost({ handleModal, header, postId, url, text }: SharePostProps) {
  //   SHARE POST
  const { mutateAsync: sharePostMutation } = useShare();

  const handleCopy = () => {
    navigator.clipboard.writeText(url);
    postId && sharePostMutation(postId);
      ...
      <div className="carousel fixed inset-0 bg-[#000000ef] z-[99] grid place-content-center p-6 ">
        <div className="rounded-2xl lg:w-[700px] w-auto  bg-white p-2 overflow-y-auto">
          ...
          <div className="flex justify-center mt-4 space-x-4">
            <TelegramShareButton
              onClick={() => postId && sharePostMutation(postId)}
              url={url}
              //   quote={"Share"}
            >
              <img
