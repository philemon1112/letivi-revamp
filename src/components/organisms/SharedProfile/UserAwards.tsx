import { fetchUserAwards } from "@/services/biography";
import { useQuery } from "@tanstack/react-query";
import React from "react";

function UserAwards({ userData }: { userData: any }) {
  const { data: awardsData, isLoading } = useQuery({
    queryKey: ["SharedUserAwardsList"],
    queryFn: async () => {
      if (!userData?.id) throw new Error("User ID is undefined");
      return await fetchUserAwards(userData.id);
    },
  });
  ...
  return (
    <div className="px-2 md:px-4 bg-white py-4 md:py-10 my-4 rounded-xl">
      ...
      <div className="px-2 md:px-4">
        ...
        {!isLoading && awardsData?.data?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center text-gray-500">
            ...
            <p className="text-sm sm:text-base">No Awards Added by User</p>
          </div>
        )}
      </div>
    </div>
