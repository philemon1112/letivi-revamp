import { fetchUserNominations } from "@/services/biography";
import { useQuery } from "@tanstack/react-query";
import React from "react";

function UserNominations({ userData }: { userData: any }) {
  const { data: nominations, isLoading } = useQuery({
    queryKey: ["SharedUserNominationList"],
    queryFn: async () => {
      if (!userData?.id) throw new Error("User ID is undefined");
      return await fetchUserNominations(userData.id);
    },
  });
    ...
    <div className="px-2 md:px-4 bg-white py-4 md:py-10 my-4 rounded-xl">
      ...
      <div className="px-2 md:px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {nominations?.data.map((nomination, index) => (
        ...
        {!isLoading && nominations?.data?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center text-gray-500">
            ...
            <p className="text-sm sm:text-base">No Nominations Added by User</p>
          </div>
        )}
      </div>
    </div>
