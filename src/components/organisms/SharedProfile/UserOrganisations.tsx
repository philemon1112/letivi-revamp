import { fetchUserOrganizations } from "@/services/biography";
import { useQuery } from "@tanstack/react-query";
import React from "react";

function UserOrganizations({ userData }: { userData: any }) {
  const { data: organizations, isLoading } = useQuery({
    queryKey: ["SharedUserOrganizationList"],
    queryFn: async () => {
      if (userData?.id === undefined) {
        throw new Error("User ID is undefined");
      }
      return await fetchUserOrganizations(userData.id);
    },
  });
          ...
          {organizations?.data.map((org, index) => (
            <div key={index} className="py-2">
              <blockquote className="flex h-full flex-col rounded-xl justify-between bg-gray-50 p-3 shadow-xs sm:p-2 lg:p-4">
                <div>
                  <div className="mt-1">
                    <p className="text-lg font-bold text-na_red sm:text-base">
                      {org?.organization}
                    </p>

                    <p className="mt-2 leading-relaxed text-gray-700">
                      {org?.role} - {org.country}
                    </p>
                  </div>
                </div>

                <footer className="mt-1 text-sm font-medium text-gray-700 sm:mt-2">
                  &mdash; {org.work_experience} years
                </footer>
              </blockquote>
            </div>
          ))}
        ...
        {!isLoading && organizations?.data.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center text-gray-500">
            ...
            <p className="text-sm sm:text-base">
              No Organisations Added by User
            </p>
          </div>
        )}
      </div>
