...
const BusinessForm = ({
  ...
  useEffect(() => {
    if (open) {
      ...
      setIndustry(business?.industry?.id || "");
      setOther_industry(business?.other_industry || "");
      if (business?.other_industry) {
        setOtherIndustry(true);
      }
      setDescription(business?.description || "");
      setWebsite(business?.business_profile?.website || "");
      setFacebook(business?.business_profile?.facebook || "");
      setInstagram(business?.business_profile?.instagram || "");
      setTwitter(business?.business_profile?.twitter || "");
      setYoutube(business?.business_profile?.youtube || "");
      setLinkedin(business?.business_profile?.linkedin || "");
      setFile(null);
      setPreview(null);
      setsearchParam("");
      setTeamMemberId([currentUser?.id]); // Reset to only the current user
  ...
  const handleSubmit = async () => {
    try {
      if (business?.id) {
        ...
        const res = await updateBusinessWorkspace(formData);
        ...
        const { data } = await createBusinessWorkspace(formData);
  ...
  return (
    <Modal
      ...
      size="2xl"
      actionButton={`${step === 1 ? "Next" : business ? "Update" : "Create"}`}
    ...
    >
      <h1 className="text-center font-medium text-lg lg:text-xl capitalize mb-2">
        {business ? `Update ${business?.name}` : "Add Organisation"} ?
      </h1>
      {step === 1 && (
