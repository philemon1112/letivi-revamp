...
const EventForm = ({
  ...
  const [industry, setIndustry] = useState<string | number | null>("");
  ...
  useEffect(() => {
    if (open) {
      ...
      setIndustry(event?.industry?.id);
      setOther_industry(event?.other_industry || "");
      if (event?.other_industry) {
        setOtherIndustry(true);
      }
      setDescription(event?.description || "");
      setWebsite(event?.event_profile?.website || "");
      setFacebook(event?.event_profile?.facebook || "");
      setInstagram(event?.event_profile?.instagram || "");
      setTwitter(event?.event_profile?.twitter || "");
      setYoutube(event?.event_profile?.youtube || "");
      setLinkedin(event?.event_profile?.linkedin || "");
      setFile(null);
      setPreview(null);
      setsearchParam("");
      setTeamMemberId([currentUser?.id]); // Reset to only the current user
  ...
  const handleSubmit = async () => {
    try {
      if (event?.id) {
        ...
        formData.append("specialize", type);
        ...
        const res = await updateEventWorkspace(formData);
        ...
        const { data } = await createEventWorkspace(formData);
  ...
  return (
    <Modal
      ...
      size="2xl"
      actionButton={`${step === 1 ? "Next" : event ? "Update" : "Create"}`}
    ...
    >
      <h1 className="text-center font-medium text-lg lg:text-xl capitalize mb-2">
        {event ? `Update ${event?.name}` : "Add event"} ?
      </h1>
      {step === 1 && (
        <div className="mx-auto">
          <form className="">
            ...
            <div className="my-2 w-full grid lg:grid-cols-2 gap-2">
              <div>
                <h1 className="mb-1  lg:text-base ">Type</h1>
                <input
                  required
                  type="text"
                  onChange={(e) => {
                  ...
                  placeholder="eg.Bazaar, Tech Conference, Press Conference, Webinar, Meetup."
                />
              </div>
              <div>
                <h1 className="mb-1  lg:text-base ">Location</h1>
                <input
                  required
                  type="text"
                  onChange={(e) => {
