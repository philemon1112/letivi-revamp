"use client";
import { getSharedEventDetails } from "@/services/workspaces";
...
import EventProfile from "../Workspace/Details/eventProfile";
import EventGallery from "../Workspace/Details/eventGallery";
...
function SharedEventDetails({ name }: { name: string }) {
  // component logic
  ...
  const {
    ...
    queryKey: ["SharedEventDetails", name],
    queryFn: () => {
      return getSharedEventDetails(name);
    },
    select: (response) => {
              ...
              <div className="md:col-span-3">
                <div className="mx-auto max-w-screen-xl mt-4">
                  <div className=" px-2 md:px-4 bg-white py-4 md:py-6 rounded-xl">
                    <div className="flex justify-between items-center align-bottom">
                      ...
                      <a
                        download
                        href={`${process.env.NEXT_PUBLIC_OLD_API_BASE}/workspaces/download/event/${event?.id}`}
                      >
                ...
                <div className="mx-auto max-w-screen-xl bg-white !rounded-xl overflow-x-auto mt-5 no-scrollbar">
                  <div className=" px-2 bg-white py-4 md:py-6 rounded-xl">
                    ...
                    <EventGallery eventId={name} type="shared" />
                  </div>
                </div>
                      ...
                      <>
                        {event?.collaborators?.length ? (
                          <div className="grid grid-cols-1 gap-8 mt-4 xl:mt-6 md:grid-cols-2 xl:grid-cols-3">
                            {event?.collaborators.map((user, index) => (
                              <div
                                key={index}
...
export default SharedEventDetails;
