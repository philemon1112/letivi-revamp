...
import { getUserWorkspaceBusinesses } from "@/services/workspaces";
import { Workspace } from "@/types/workspaces";
import { useInfiniteQuery } from "@tanstack/react-query";
import React, { useEffect, useState } from "react";
import { useInView } from "react-intersection-observer";

function Businesses({ userId }: { userId: number }) {
  ...
  const {
    ...
    queryKey: ["UserPersonalBusinessList", userId],
    queryFn: async ({ pageParam = 1 }) => {
      if (!userId) throw new Error("User ID is required");
      return await getUserWorkspaceBusinesses(userId, {
        page: pageParam,
        limit: pageSize,
      });
    },
  ...
  const allBusinesses =
    data?.pages?.flatMap(
      (page) =>
        page?.data?.map((business: Workspace) => ({
          ...business,
          type: "business",
        })) ?? [],
    ) ?? [];
              ...
              <>
                ...
                {allBusinesses.length ? (
                  <div className="grid lg:grid-cols-3 sm:grid-cols-2 gap-10 p-4 justify-center">
                    {allBusinesses.map((workspace, index) => (
                      <WorkspaceCard
                        type="personal"
