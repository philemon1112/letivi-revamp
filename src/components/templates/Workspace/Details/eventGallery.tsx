...
function EventGallery({
  ...
  const {
    ...
    queryKey: ["EventDetailsPost", type, eventId], // Include type in queryKey to refetch when type changes
    queryFn: ({ pageParam = 1 }) => {
      return type === "shared"
        ? getSharedEventPosts(eventId, {
            page: pageParam,
            limit: pageSize,
          })
        : getEventsPosts(eventId, { page: pageParam, limit: pageSize });
    },
          ...
          <>
            {allPosts?.length > 0 ? (
              ...
              <>
                ...
                <PostEmptyState title="No Events Post yet" />
              </>
            )}
          </>
        )}
