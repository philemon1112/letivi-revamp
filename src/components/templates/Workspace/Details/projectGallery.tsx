...
import { getProjectsPosts, getSharedProjectPosts } from "@/services/workspaces";
...
function ProjectGallery({
  ...
  const {
    ...
    queryKey: ["ProjectDetailsPost", type, projectId],
    queryFn: ({ pageParam = 1 }) => {
      return type === "shared"
        ? getSharedProjectPosts(projectId, {
            page: pageParam,
            limit: pageSize,
          })
        : getProjectsPosts(projectId, { page: pageParam, limit: pageSize });
    },
          ...
          <>
            {allPosts?.length > 0 ? (
              ...
              <>
                ...
                <PostEmptyState title="No Projects Post yet" />
              </>
            )}
          </>
        )}
